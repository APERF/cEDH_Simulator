...
class StackObject:
    ...
    def to_dict(self, controller_name: str = "") -> dict:
        name = (
            f"{self.card.name}: {self.ability_description}"
            if self.is_ability and self.ability_description
            else self.card.name
        )
        return {
            "id": self.id,
...
class Stack:
    ...
    def resolve_top(self, game_state: GameState) -> StackObject | None:
        ...
        if obj and obj.resolve_fn:
            ...
            if obj.is_ability:
                # Activated/triggered ability: source card stays in its zone; just flush triggers
                game_state.flush_effect_queue()
            else:
                # Spell: determine outcome by where the card ended up
